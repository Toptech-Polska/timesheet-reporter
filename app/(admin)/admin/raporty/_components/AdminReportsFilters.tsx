...
export function AdminReportsFilters({ selectedYear, selectedMonth, selectedStatus }: Props) {
  ...
  const update = useCallback(
    (key: string, value: string) => {
      ...
      router.push(`/admin/raporty?${params.toString()}`);
    },
    [router, searchParams]
  );
  ...
  const isDefault = selectedYear === thisYear && selectedMonth === 0 && selectedStatus === "all";
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      {!isDefault && (
        <Button variant="outline" size="sm" onClick={() => router.push(`/admin/raporty?rok=${thisYear}`)}>
          Resetuj filtry
        </Button>
      )}
    </div>
