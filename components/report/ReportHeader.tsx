...
export function ReportHeader({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-6 py-5 print:shadow-none print:border-0">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <div className="text-sm text-slate-700 space-y-0.5">
            ...
            {!!profile?.contractor_nip && (
              <p className="text-slate-500">NIP: {str(profile.contractor_nip)}</p>
            )}
            {!!profile?.contractor_address && (
              <p className="text-slate-500">{str(profile.contractor_address)}</p>
            )}
        ...
        <div>
          ...
          <div className="text-sm text-slate-700 space-y-0.5">
            ...
            {!!settings?.client_nip && (
              <p className="text-slate-500">NIP: {str(settings.client_nip)}</p>
            )}
            {!!settings?.client_address && (
              <p className="text-slate-500">{str(settings.client_address)}</p>
            )}
          </div>
        </div>
      </div>
