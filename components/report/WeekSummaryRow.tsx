...
export function WeekSummaryRow({
  ...
  return (
    <tr className="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
      <td
        colSpan={4}
        className="px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300"
      >
        Suma tygodnia {weekNum} ({isoToShort(startDate)}–{isoToShort(endDate)}):
      </td>
      <td className="px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 text-right tabular-nums">
        {formatHours(hours)}
      </td>
      <td />
      <td className="px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 text-right tabular-nums">
